...
function getApiConfig() {
    ...
    const baseUrl = (config.get('baseUrl') || 'https://api.deepseek.com').replace(/\/+$/, '');
    const deepseekModel = config.get('model') || 'deepseek-v4-flash';
    const configuredMaxTokens = config.get('maxTokens') || 4096;
...
function getModelResponseContent(jsonData) {
    ...
    return ((_c = (_b = (_a = jsonData.choices) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.delta) === null || _c === void 0 ? void 0 : _c.content) || ((_f = (_e = (_d = jsonData.choices) === null || _d === void 0 ? void 0 : _d[0]) === null || _e === void 0 ? void 0 : _e.message) === null || _f === void 0 ? void 0 : _f.content) || '';
}
function getModelFinishReason(jsonData) {
    var _a, _b, _c, _d;
    return ((_b = (_a = jsonData.choices) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.finish_reason) || ((_d = (_c = jsonData.choices) === null || _c === void 0 ? void 0 : _c[0]) === null || _d === void 0 ? void 0 : _d.finishReason) || '';
}
// 获取 deepseek 回复 非流式
...
function getDeepSeekResponse(viewProvider, prompt, onProgress) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!apiKey) {
            vscode.window.showErrorMessage((0, localize_1.localize)('error.apiKeyRequired', 'Please set the DeepSeek API key.'));
            return;
        }
        console.log('DeepSeek 请求开始');
        ...
        try {
            ...
            if (!response.ok) {
                ...
                fullResponse += `\n\n${(0, localize_1.localize)('response.errorWithReason', 'Request error: {0}', errorMessage)}`;
                onProgress(fullResponse);
                vscode.window.showErrorMessage((0, localize_1.localize)('error.requestNotification', 'DeepSeek Code Generator: {0}', errorMessage));
            ...
            const processStreamLine = (line) => __awaiter(this, void 0, void 0, function* () {
                var _a;
                ...
                try {
                    ...
                    if (jsonData.error || jsonData.code === '50501') {
                        const requestErrorMessage = ((_a = jsonData.error) === null || _a === void 0 ? void 0 : _a.message) || jsonData.message || JSON.stringify(jsonData);
                        fullResponse += `\n\n${(0, localize_1.localize)('response.errorWithReason', 'Request error: {0}', requestErrorMessage)}`;
                        console.error('Request error:', requestErrorMessage);
                        vscode.window.showErrorMessage((0, localize_1.localize)('error.requestNotification', 'DeepSeek Code Generator: {0}', requestErrorMessage));
                        onProgress(fullResponse);
                        yield reader.cancel();
                        return false;
                    }
                    ...
                    if (finishReason === 'length') {
                        fullResponse += `\n\n${(0, localize_1.localize)('response.maxTokensReached', 'The response stopped because it reached the configured max token limit. Increase DeepSeek: Max Tokens in Settings and try again.')}`;
                        onProgress(fullResponse);
                        return false;
                    }
                    if (stopRequest) {
                        fullResponse += `\n\n${(0, localize_1.localize)('response.userStopped', 'User stopped the request')}`;
                        onProgress(fullResponse);
                        stopRequest = false;
            ...
            while (true) {
                ...
                for (const line of lines) {
                    if (!(yield processStreamLine(line))) {
            ...
            console.error('DeepSeek 请求失败:', error);
            viewProvider.showLoading(false);
            onProgress((0, localize_1.localize)('response.requestFailed', 'Request failed'));
            stopRequest = false;
        }
        finally {
...
function activate(context) {
    ...
    const { apiKey } = getApiConfig();
    if (!apiKey) {
        const configureApiKeyMessage = (0, localize_1.localize)('info.apiKeyMissing', 'API keys are not set. Click here to configure them.');
        const openSettingsLabel = (0, localize_1.localize)('action.openSettings', 'Open Settings');
        vscode.window.showInformationMessage(configureApiKeyMessage, { modal: true }, openSettingsLabel).then((selection) => {
    ...
    let disposable = vscode.commands.registerCommand('extension.deekseek', () => __awaiter(this, void 0, void 0, function* () {
        // 获取用户输入
        const input = yield vscode.window.showInputBox({ prompt: (0, localize_1.localize)('input.prompt.question', 'Please enter your question') });
        if (input) {
    ...
    const selectionCommandDisposables = [
        ...
        return vscode.commands.registerCommand(command, () => __awaiter(this, void 0, void 0, function* () {
            ...
            yield vscode.commands.executeCommand('workbench.view.extension.deepseekContainer');
    ...
    const view = vscode.window.createTreeView('deepseekExplorer', {
        treeDataProvider: {
            ...
            getTreeItem: () => {
                return {
                    label: (0, localize_1.localize)('tree.settingsLabel', 'DeepSeek Settings'),
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('deepseek.apiKey')) {
            ...
            if (apiKey) {
                vscode.window.showInformationMessage((0, localize_1.localize)('info.apiKeyUpdated', 'DeepSeek API key updated!'));
            }
        }
    });
    console.log('DeepSeek 插件已激活');
}
const selectionPromptLabels = {
...
const selectionPromptInstructions = {
...
function buildSelectionPrompt(action) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage((0, localize_1.localize)('warning.noActiveEditor', 'No active editor.'));
        return undefined;
    }
    const selection = editor.selection;
    const selectedCode = editor.document.getText(selection);
    if (!selectedCode.trim()) {
        vscode.window.showWarningMessage((0, localize_1.localize)('warning.noSelectedCode', 'Please select code first.'));
        return undefined;
    }
...
function getResponseTextForEditor(response) {
    const codeBlockMatch = response.match(/```[^\n\r]*[\n\r]+([\s\S]*?)```/);
    return ((codeBlockMatch === null || codeBlockMatch === void 0 ? void 0 : codeBlockMatch[1]) || response).trim();
}
// Webview 提供者类
...
class DeepSeekWebviewProvider {
    ...
    askDeepSeek(prompt) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let updateTimer;
            const renderCurrentResponse = (showLoading) => {
            ...
            yield getDeepSeekResponse(this, prompt, (text) => {
    ...
    updateWebView(showLoading = false) {
        if (this._view) {
            ...
            const localizedStrings = {
                chatHeading: (0, localize_1.localize)('webview.heading', 'DeepSeek'),
                tipLabel: (0, localize_1.localize)('webview.tip.label', 'AI Chat'),
                historyUserLabel: (0, localize_1.localize)('webview.history.userLabel', 'You:'),
                historyAssistantLabel: (0, localize_1.localize)('webview.history.assistantLabel', 'DeepSeek:'),
                historyEmpty: (0, localize_1.localize)('webview.history.empty', 'No history yet'),
                chatUserLabel: (0, localize_1.localize)('webview.chat.userLabel', 'You'),
                chatAssistantLabel: (0, localize_1.localize)('webview.chat.assistantLabel', 'DeepSeek'),
                chatEmpty: (0, localize_1.localize)('webview.chat.empty', 'No messages yet. Start a new conversation.'),
                chatHistoryTooltip: (0, localize_1.localize)('webview.tooltip.chatHistory', 'Chat History'),
                settingsTooltip: (0, localize_1.localize)('webview.tooltip.settings', 'Settings'),
                newChatTooltip: (0, localize_1.localize)('webview.tooltip.newChat', 'New Chat'),
                historyPanelTitle: (0, localize_1.localize)('webview.history.title', 'Latest 10 conversations'),
                historyPanelCloseAria: (0, localize_1.localize)('webview.history.closeAria', 'Close history panel'),
                historyClearAllTitle: (0, localize_1.localize)('webview.history.clearTitle', 'Clear all history'),
                historyClearAllLabel: (0, localize_1.localize)('webview.history.clearLabel', 'Clear all history'),
                generatingLabel: (0, localize_1.localize)('webview.progress.generating', 'Generating'),
                stopLabel: (0, localize_1.localize)('webview.progress.stop', 'Stop'),
                modelLabel: (0, localize_1.localize)('webview.model.label', 'Model'),
                copyResponseLabel: (0, localize_1.localize)('webview.action.copyResponse', 'Copy'),
                insertResponseLabel: (0, localize_1.localize)('webview.action.insertResponse', 'Insert'),
                replaceSelectionLabel: (0, localize_1.localize)('webview.action.replaceSelection', 'Replace'),
                newFileLabel: (0, localize_1.localize)('webview.action.newFile', 'New File'),
                sendLabel: (0, localize_1.localize)('webview.action.send', 'Send'),
                textareaPlaceholder: (0, localize_1.localize)('webview.input.placeholder', 'Input your question, e.g. Generate a countdown JavaScript code...'),
                confirmClearHistory: (0, localize_1.localize)('webview.confirm.clearHistory', 'Are you sure you want to clear all chat history? This action cannot be undone.')
            };
    ...
    getEditorReadyResponse(uuid) {
        var _a;
        const entry = this.findChatEntry(uuid);
        if (!((_a = entry === null || entry === void 0 ? void 0 : entry.DeepSeek) === null || _a === void 0 ? void 0 : _a.trim())) {
            vscode.window.showWarningMessage((0, localize_1.localize)('warning.noResponseContent', 'No response content available.'));
            return undefined;
        }
    ...
    copyResponse(uuid) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const entry = this.findChatEntry(uuid);
            const text = (_a = entry === null || entry === void 0 ? void 0 : entry.DeepSeek) === null || _a === void 0 ? void 0 : _a.trim();
            if (!text) {
                vscode.window.showWarningMessage((0, localize_1.localize)('warning.noResponseContent', 'No response content available.'));
                return;
            }
            try {
                ...
                vscode.window.showInformationMessage((0, localize_1.localize)('info.responseCopied', 'Response copied.'));
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                vscode.window.showWarningMessage(`${(0, localize_1.localize)('warning.copyResponseFailed', 'Failed to copy response.')} ${message}`);
            }
        });
    }
    insertResponse(uuid) {
    ...
    newFileFromResponse(uuid) {
        return __awaiter(this, void 0, void 0, function* () {
            const text = this.getEditorReadyResponse(uuid);
            if (!text) {
                return;
            }
            const document = yield vscode.workspace.openTextDocument({ content: text });
    ...
    handleClearHistory() {
        return __awaiter(this, void 0, void 0, function* () {
            const confirmMessage = (0, localize_1.localize)('webview.confirm.clearHistory', 'Are you sure you want to clear all chat history? This action cannot be undone.');
            const yesLabel = (0, localize_1.localize)('action.yes', 'Yes');
            const noLabel = (0, localize_1.localize)('action.no', 'No');
            const selection = yield vscode.window.showWarningMessage(confirmMessage, { modal: true }, yesLabel, noLabel);
